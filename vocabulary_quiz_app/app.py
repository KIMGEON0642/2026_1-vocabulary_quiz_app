...
class VocabularyQuizApp:
    def __init__(self, root: tk.Tk, words: list[Word]) -> None:
        ...
        # 중복 없는 출제를 위한 단어 순서 생성
        self.word_order = words.copy()
        self.rng.shuffle(self.word_order)
        self.word_index = 0
        ...
        root.bind("<Return>", self.on_enter_pressed)
    ...
    def next_word(self) -> None:
        # 모든 단어를 다 출제한 경우
        if self.word_index >= len(self.word_order):
            self.word_var.set("학습 완료!")

            self.feedback_var.set(
                f"최종 점수: {self.score}/{self.total}"
            )
